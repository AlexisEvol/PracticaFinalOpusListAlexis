...
public class MainForm extends javax.swing.JFrame {
    ...
    private void btnSaveActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnSaveActionPerformed
        ...
        JsonWriter writer;
        try {
            writer = new JsonWriter(new FileWriter(System.getProperty("user.home") + "\\AppData\\Local\\OpusList\\obres.json"));
            getLista();
            writeDoublesArray(writer, lista);
    ...
    public void writeDoublesArray(JsonWriter writer, List<Obra> obras) throws IOException {
     writer.beginArray();
     for (Obra obra : lista) {
       writeMessage(writer, obra);
     }
     writer.endArray();
   }
    
   public void writeMessage(JsonWriter writer, Obra obras) throws IOException {
     writer.beginObject();
     writer.name("registre").value(obras.getRegistre());
     writer.name("titol").value(obras.getTitol());
     writer.name("year").value(obras.getAny());
     writer.name("format").value(obras.getFormat());
     writer.name("autor").value(obras.getAutor());
     writer.name("autor").value(obras.getAutor());
     writer.name("imatge").value(obras.getImagen());
     writer.endObject();
   }
